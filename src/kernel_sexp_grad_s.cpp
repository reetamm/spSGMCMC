...
double d_kernel_sexp_s(const arma::vec &x, const arma::vec &y, const double l, const double s) {
  ...
  if (equal_matrices == 1) {
    ...
    for (int r = 0; r < M.n_rows; r++) {
      for (int c = r + 1; c < M.n_rows; c++) {
        K(r, c) = d_kernel_sexp_s(M.row(r).t(), M.row(c).t(), l, s);
      }
    }
    K = K + K.t();
    // fill diag
    ...
    for (int r = 0; r < M.n_rows; r++) {
      for (int c = 0; c < N.n_rows; c++) {
        K(r, c) = d_kernel_sexp_s(M.row(r).t(), N.row(c).t(), l, s);
      }
    }
  }
