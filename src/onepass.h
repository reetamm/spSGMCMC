...
#pragma omp for
  for(int i=0; i<n; i++){
    ...
    int bsize = std::min( batch_id[i],m);
    ...
    for(int j=bsize-1; j>=0; j--){
      ysub(bsize-1-j) = y( NNarray(batch_id[i]-1,j)-1 );
      for(int k=0;k<dim;k++){ locsub(bsize-1-j,k) = locs( NNarray(batch_id[i]-1,j)-1, k ); }
      if(profbeta){
        for(int k=0;k<p;k++){ X0(bsize-1-j,k) = X( NNarray(batch_id[i]-1,j)-1, k ); }
      }
    }
...
void  my_synthesize(
    ...
    NumericVector* grad,
    NumericVector* grad_t,
    NumericVector* grad_beta,
    NumericMatrix* info,
  ...
  //arma::vec batch_id_c = arma::vec(batch_id.begin(), batch_id.length());
  ...
  my_compute_pieces(
    batch_id, covparms_c, covfun_name, locs_c, NNarray_c, y_c, X_c,
  ...
  // betahat and dbeta
  arma::vec beta_c = arma::vec(current_beta.begin(), current_beta.length(), false);
  ...
  if(grad_info){
    ...
    for(int j=0; j<nparms; j++){
      ...
      (*grad_t)(j) -= 0.5*dlogdet(j);
      (*grad)(j) += 0.5*dySy(j);
      (*grad_t)(j) += 0.5*dySy(j);
      (*grad)(j) -= 1.0*as_scalar( beta_c.t() * dySX.col(j) );
      (*grad_t)(j) -= 1.0*as_scalar( abeta.t()  * dySX.col(j) );
      (*grad)(j) += 0.5*as_scalar( beta_c.t() * dXSX.slice(j) * abeta );
      (*grad_t)(j) += 0.5*as_scalar(abeta.t() * dXSX.slice(j) * abeta );
      (*grad_t)(j) += 1.0*as_scalar( ySX.t() * dbeta.col(j) );
      (*grad_t)(j) -= 1.0*as_scalar( abeta.t() * XSX * dbeta.col(j) );
    }
    arma::vec grad_beta_c = ySX - XSX*beta_c;
    for(int j=0; j<p; j++){
      (*grad_beta)(j) = grad_beta_c(j); //ySX(j) - as_scalar(current_beta.t()*XSX)
    }
    // fisher information
    for(int i=0; i<nparms; i++){ for(int j=0; j<i+1; j++){
      (*info)(i,j) = ainfo(i,j);
